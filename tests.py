...
class TestAppIdFromTitle:
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("ADD com.example.App", "com.example.App"),
            ("add com.example.App", "com.example.App"),
            ("Add com.example.foobar", "com.example.foobar"),
...
class TestParseChecklist:
    ...
    def test_parses_unchecked_items(self):
        assert parse_checklist("- [ ] Do something\n") == [(False, "Do something")]
...
class TestChecklistMatchesTemplate:
    ...
    def test_missing_role_item_does_not_match(self):
        checklist = parse_checklist(MISSING_ITEM_CHECKLIST_BODY)
        assert checklist_matches_template(checklist) is True
...
class TestCountUncheckedRelevantItems:
    ...
    def test_unrelated_items_are_not_counted(self):
        checklist = parse_checklist("- [x] Buy milk\n- [ ] Walk the dog\n")
        assert count_unchecked_relevant_items(checklist) == 0

    def test_empty_checklist_has_zero_unchecked(self):
        checklist = parse_checklist(NO_CHECKLIST_BODY)
        assert count_unchecked_relevant_items(checklist) == 0


class TestIsConsideredSpam:
    def test_valid_submission_is_not_spam(self):
        assert is_considered_spam(VALID_FILES, FULL_CHECKLIST_BODY) is False

    def test_all_files_in_subdirectory_is_spam(self):
        nested_files = ["some/nested/file.json", "another/nested/file.yaml"]
        assert is_considered_spam(nested_files, FULL_CHECKLIST_BODY) is True

    def test_missing_checklist_template_is_spam(self):
        assert is_considered_spam(VALID_FILES, NO_CHECKLIST_BODY) is True

    def test_incomplete_checklist_template_is_spam(self):
        assert is_considered_spam(VALID_FILES, MISSING_ITEM_CHECKLIST_BODY) is False

    @pytest.mark.parametrize("unchecked", [2, 3, 4])
    def test_too_many_unchecked_items_is_spam(self, unchecked):
        assert (
            is_considered_spam(VALID_FILES, checklist_body(unchecked=unchecked)) is True
        )

    def test_one_unchecked_item_is_not_spam(self):
        assert is_considered_spam(VALID_FILES, checklist_body(unchecked=1)) is False

    def test_unchecked_boundary_matches_max_allowed_constant(self):
        assert MAX_UNCHECKED_ITEMS_ALLOWED == 1

    def test_nested_files_override_valid_checklist(self):
        nested_files = ["some/nested/file.json", "another/nested/file.yaml"]
        assert is_considered_spam(nested_files, FULL_CHECKLIST_BODY) is True
...
class TestIsReviewCommand:
    ...
    def test_matches_exact_command(self, text, expected):
        assert is_review_command(text) is expected
...
class TestHasMasterCommit:
    def test_detected_as_second_commit(self):
        ...
        inner3 = FakeCommitInner(
            email="foobar@example.org", message="A foobar commit headline"
        )
        pr = make_fake_raw_pr(
            commits=[
                FakeRawCommit(inner1),
                FakeRawCommit(inner2),
                FakeRawCommit(inner3),
            ]
        )
...
class TestLeaveOpenLabel:
    ...
    def test_pr_without_leave_open_label_is_processed_normally(self):
        raw_pr = make_fake_raw_pr(
            title="Add com.example.foobar",
            body=FULL_CHECKLIST_BODY,
...
class TestHasMissingVideo:
    ...
    def test_na_on_next_line_is_missing(self):
        body = (
            "- [x] Please attach a video showcasing the application on Linux "
            "using the Flatpak.\nNo video available\n"
        )
...
class TestValidatePR:
    ...
    def test_domain_comment_posted_for_verifiable_appid(self):
        raw_pr = make_fake_raw_pr(
            title="Add com.example.foobar",
            body=FULL_CHECKLIST_BODY,
            files=["com.example.foobar.json"],
        ...
        comment_bodies = [c[0][1] for c in client.post_comment.call_args_list]
        assert any(DOMAIN_COMMENT_PARTIAL in b for b in comment_bodies)
        assert any("example.com" in b for b in comment_bodies)
